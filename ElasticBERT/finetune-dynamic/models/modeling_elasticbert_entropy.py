...
class ElasticBertEncoder(nn.Module):
    def __init__(self, config, add_pooling_layer=None):
        ...
        assert sum(self.num_output_layers) <= self.num_hidden_layers, \
        ...
        if sum(self.num_output_layers) > 1:
            #self.start_output_layer = self.num_hidden_layers - sum(self.num_output_layer)
            #start_pooler_num = self.start_output_layer
            #end_pooler_num = self.num_hidden_layers - 1
            if add_pooling_layer:
                self.pooler = nn.ModuleList([ElasticBertPooler(config) if i \
                                                 else None for i in self.num_output_layers])
        elif sum(self.num_output_layers) == 1:
    ...
    def forward(
        ...
        output_sequence_outputs = () if sum(self.num_output_layers) > 1 else None
        output_pooled_outputs = () if sum(self.num_output_layers) > 1 else None
    ...
    def adaptive_forward(
        ...
        pooled_output = None
        if self.add_pooling_layer and self.num_output_layers[current_layer] == 1:
    ...
    def forward(
        ...
        if self.training:
            ...
            for i in range(self.num_hidden_layers):
                ...
                logits = None
                if self.add_pooling_layer and self.num_output_layers[i] == 1:
                    assert pooled_output is not None
                    logits = output_layers[i](output_dropout(pooled_output))
                    res.append(logits)
                elif not self.add_pooling_layer and self.num_output_layers[i] == 1:
            ...
            assert len(res) == sum(self.num_output_layers)
            ...
            assert len(pooled_outputs) == sum(x is not None for x in output_layers)

            res = []

            for i, pooled_output in enumerate(pooled_outputs):
                logit = [x for x in output_layers if x][i](pooled_output)
                res.append(logit)
        else:
            middle_result = None
...
class ElasticBertForSequenceClassification(ElasticBertPreTrainedModel):
    def __init__(self, config, add_pooling_layer=True):
        ...
        self.classifiers = nn.ModuleList([nn.Linear(config.hidden_size, self.config.num_labels) if i else None for i in config.num_output_layers])
