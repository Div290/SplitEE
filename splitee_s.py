...
def UCB(df,threshhold,lembda,o,n_arms=12,n_epoch=5,c=1):
    ...
    Returns:
        ...
        number of sample offloaded in each epoch
        cost of each epoch
        """
    rewards_list_final=[]
    accuracy_list=[]
    ...
    for _ in range(n_epoch):
        ...
        for trial in range(df.shape[0]):
            ...
            for pr_arm in range(best_arm):
                reward,__,__,__=reward_cost_yPred_generator(x[trial],threshhold,pr_arm,lembda,o,is_best_arm=False)
    ...
    return (np.argmax(np.mean(rewards_list_final,axis=0)),accuracy_list,sample_offloaded,cost_list)

#To run below file please run Finetuning_of_ElasticBERT_over_datasets.ipynb file first
df_results_dict=pd.read_pickle('path../confidence_label_dict.pkl')
...
for dataset in dataset_name:
    ...
    for offload_cost in tqdm(o_value):
        result[offload_cost]=tuple(UCB(df,threshhold[dataset],lembda,offload_cost,n_arms=12,n_epoch=20,c=1))
    result_dict[dataset]=result

with open("path../splitEE-S.pkl",'wb') as file:
    pickle.dump(result_dict,file)

# for getting result for single dataset and offloading cost run below code
# best_arm,accuracy,offloaded,cost=UCB(df_results_dict[sys.argv[1]],threshhold[sys.argv[1]],lembda,float(sys.argv[2]),n_arms=12,n_epoch=5,c=1)
